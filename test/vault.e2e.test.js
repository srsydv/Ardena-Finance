...
// //         // add other funcs/events you need
// //       ];
// //       const exchangerContract = await ethers.getContractAt(EXCHANGE_ABI, exchanger.target);
...
// //       await WETH1.deposit({ value: ethers.parseEther("1") });
// //       // Transfer WETH to the strategy address
// //       const amountToGive = ethers.parseEther("0.5"); // 0.5 WETH
// //       await WETH1.transfer(uniStrat.target, amountToGive);

// //       // === STEP 2: impersonate the strategy address so it can approve the handler ===
// //     await network.provider.request({ method: "hardhat_impersonateAccount", params: [uniStrat.target] });
// //     uniStratSigner = await ethers.getSigner(uniStrat.target);
...
// //       usdc.target,
// //       500,
// //       exchanger.target /* recipient: send the output USDC to the strategy (or to uniStrat.address if desired) */,
// //       deadline,
// //       amountIn,
// //       amountOutMinimum,
// //       sqrtPriceLimitX96
...
// //        [SWAP_ROUTER_V3, WETH, usdc.target, amountIn, amountOutMinimum, exchanger.target /* recipient */, routerCalldata]
// //      );

//     //  await exchangerContract.setRouter(SWAP_ROUTER_V3, true);

// //       // === STEP 5: simulate callStatic first (recommended) ===
...
// // console.log("exchanger.address:", exchanger.target);
// //       const callStaticResult = await exchangerContract.connect(uniStratSigner).swap(payload);
// //       console.log("callStatic swap returned (amountOut):", callStaticResult.toString());
// //     } catch (e) {
// //       console.error("callStatic simulation failed - inspect message:", e.message);
// //       throw e;
// //     }

// //     // === STEP 6: execute the actual swap TX ===
// //     const tx = await exchangerContract.connect(uniStratSigner).swap(payload);
...
//       await WETH9.transfer(exchanger.target, amountToSend);
...
//       //     WETH,
//       //     usdc.target,
//       //     newbal.toString(), // note: ExchangeHandler will override with balance at runtime
//       //     quote.minBuyAmount, // safe minimum
//       //     uniStrat.target,
//       //     quote.transaction.data, // calldata to send to router
//       //   ]
...
//         const vaultBefore = await usdc.balanceOf(vault.target);
//         console.log("vaultBefore", vaultBefore.toString());

//         const uniStratweth = await weth.balanceOf(uniStrat.target);
//         console.log("before harvest uniStratweth", uniStratweth.toString());

//         const b = await ethers.provider.getBlock("latest");
// console.log("block.gasLimit:", b.gasLimit.toString());

//         const balanceWei = await ethers.provider.getBalance(uniStrat.target);
//         console.log("uni eth balanceWei", balanceWei.toString());

//         await vault.harvestAll(allHarvest,{ gasLimit: 90_000_000 });

//         const vaultAfter = await usdc.balanceOf(vault.target);
//         console.log("vaultBefore", vaultAfter.toString());

//         const uniStratWeth = await weth.balanceOf(uniStrat.target);
//         console.log("After harvest uniStratweth", uniStratWeth.toString());

//         const balanceWeiAfter = await ethers.provider.getBalance(uniStrat.target);
//         console.log("uni eth balanceWei after", balanceWeiAfter.toString());
