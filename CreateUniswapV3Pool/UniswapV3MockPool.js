...
async function main() {
  ...
  const [t0, t1] = WETH.toLowerCase() < USDC.toLowerCase()
    ? [WETH, USDC]
    : [USDC, WETH];
  ...
  const [sym0, sym1, dec0, dec1] = await Promise.all([
    t0c.symbol(), t1c.symbol(), t0c.decimals(), t1c.decimals(),
  ]);
  ...
  if (SEED_USDC !== undefined || SEED_WETH_WEI !== undefined) {
    ...
    if (SEED_WETH_WEI) await (await cW.approve(NFP_MANAGER, SEED_WETH_WEI)).wait();
    if (SEED_USDC) await (await cU.approve(NFP_MANAGER, SEED_USDC)).wait();

    // map desired amounts to amount0/1 for mint params
    const amount0Desired = (t0.toLowerCase() === USDC.toLowerCase()) ? (SEED_USDC ?? 0n) : (SEED_WETH_WEI ?? 0n);
    const amount1Desired = (t1.toLowerCase() === USDC.toLowerCase()) ? (SEED_USDC ?? 0n) : (SEED_WETH_WEI ?? 0n);
